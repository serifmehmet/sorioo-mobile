...
class EmailRegisterLastStepView extends ConsumerStatefulWidget {
  const EmailRegisterLastStepView({required this.args, super.key});
  ...
  Future<void> _submit() async {
    ...
    if (formKey.currentState!.validate()) {
      ...
      if (success && context.mounted) {
        await CacheManager.instance.setStringValue(
          PreferencesKeys.password,
          password,
        );
        context.navigator.pushRegisterVerify(
          EmailVerifyPageArgs(email: widget.args.email),
        );
      }
    }
  }
